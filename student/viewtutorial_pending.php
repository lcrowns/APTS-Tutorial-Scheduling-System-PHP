...
$stmt = $tutorial->viewTutorials('',$name,'Pending');
				...
				if($srch->rowCount()>0){
  					while($row=$srch->fetch(PDO::FETCH_ASSOC)){
						...
						<tr onclick=\"window.location='viewonetutorialreq.php?tlistId={$tlistId}'\">
							...
							<td class='text-center'><a class='btn btn-secondary' href='cancelreq.php?tlistId={$tlistId}' role='button'>Cancel Request</a></td>
						</tr>";
					}									
				}else{
				echo "
