	...
	<table class='table table-hover'>
		...
		<tbody>";
			if(isset($_POST['search'])){
				$srch = $tutorial->searchTutorials2($name,$_POST['search'],'Pending');			  	
				if($srch->rowCount()>0){
  					while($row=$srch->fetch(PDO::FETCH_ASSOC)){
						...
						<tr onclick=\"window.location='viewonetutorialreq.php?tlistId={$tlistId}'\">
							...
							<td class='text-center'><a class='btn btn-secondary' href='changereqstat.php?tlistId={$tlistId}&set=Accepted' role='button'>Accept</a> <a class='btn btn-secondary' href='changereqstat.php?tlistId={$tlistId}&set=Declined' role='button'>Decline</a></td>
						</tr>";
					}									
				}else{
				echo "
